    ...
    { //  if the new position is occupied
        ...
        else if (isdigit(other_player->c))
        {
            ...
            if (player->health < INITIAL_HEALTH)
            {
                ...
                int prize = atoi(other_player->c); // alternative (don't know if it works)
            ...
            // IF THE OTHER IS A BOT
        }
        else
        {
            // decrements moving player health
            player->health--;
        }
    }
...
position_t initialize_bot(int n_bots, client_list *head)
{
    srand(time(0));
    position_t init_pos;
    client_list *search_pos = NULL;
