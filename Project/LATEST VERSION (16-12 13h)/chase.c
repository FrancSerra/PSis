...
int health_0(client_list* head, client_list* player, WINDOW* win) {
    ...
    if (player->health == 0) {
        delete_client_err = delete_client(head, player->pid, win);
...
int update_client(client_list *head, int pid, int direction, WINDOW *win){
    ...
    player = search_client(head, pid); // returns NULL if not found
    if (player == NULL)
        return -1;

    int x, y, err, is_health0;      // aux
    ...
    if (other_player == NULL || other_player == player){ // If the new_pos is empty or if it is its current position
    ...
    else{ //  if the new position is occupied

        // IF THE OTHER IS A PLAYER
        if (isalpha(other_player->c) != 0){
            
            // increments moving player health
        ...
        // if the other is a bot, nothing happens
    }
...
client_list* update_bot(client_list *head, client_list* aux, int mod, WINDOW* win) {
    ...
    for (client_list* temp = aux->next; temp != NULL; temp = temp->next)
    {
        if (temp->c == BOT_CHAR){
            ...
            // mod: 1 - up, 2 - down, 3 - left, 4 - right            
            if (mod == 1){
                if (y != 1){
                    y--;
                }
            ...
            if (other_client == NULL || other_client == temp){ // If the new_pos is empty or if it is its current position
            ...
            return temp; // returns temp if found
        }
    }
    return NULL;
}
...
position_t* update_field(client_list* head){
    ...
    position_t* arr_field = (position_t *) malloc(sizeof(position_t)*(MAX_PLAYERS+MAX_BOTS+MAX_PRIZES)+1);

    client_list* temp = head->next;
    int i = 0;

    while(temp != NULL) {
        arr_field[i].c = temp->c;
        arr_field[i].x = temp->x;
        arr_field[i].y = temp->y;
        arr_field[i].health = temp->health;
...
position_t initialize_bot_prizes(client_list *head, int bot)
{
    ...
    { // temp if found; NULL if not found
        init_pos.x = (rand() % (WINDOW_SIZE - 2 - 1 + 1)) + 1;
        init_pos.y = (rand() % (WINDOW_SIZE - 2 - 1 + 1)) + 1;
    }
...
void move_client (client_list* client, WINDOW* win, int x, int y){
    ...
    // we have to pass a struct, not  apointer, so:
    new_play.c = client->c;
    new_play.x = client->x;
    new_play.y = client->y;
