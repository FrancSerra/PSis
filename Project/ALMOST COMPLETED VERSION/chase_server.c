		...
		if(n_bytes == sizeof(message_t)) {
            ...
            switch(in_msg.type) {
                ...
                case ball_mov:
                    ...
                    if (err_update == -1) {
                        out_msg_ballmov = msg2send_ballmov(error, in_msg.pid, NULL);
                    }
                    else {
                        arr_field = update_field(head);
                        out_msg_ballmov = msg2send_ballmov(field_stat, in_msg.pid, arr_field);
                    }
                ...
                case bot_conn:
                    ...
                    if (num_bots < MAX_BOTS){
                        ...
                        for (int i = 0; i < n_bots; i++){
                            ...
                            new_client_err = insert_new_client(head, in_msg.pid, init_pos.c, init_pos.x, init_pos.y, -1); // health doesn't have a meaning for the bot
                            if (new_client_err != -1) {
                                draw_player(my_win, &init_pos, true);
                                num_bots++;
                            }
                ...
                case prizes_conn:
                    ...
                    if (num_prizes < MAX_PRIZES) {
                        ...
                        for (int i = 0; i < n_prizes; i++){
                            ...
                            new_client_err = insert_new_client(head, in_msg.pid, init_pos.c, init_pos.x, init_pos.y, -1); // health doesn't have a meaning for the prizes
                            if (new_client_err != -1) {
                                draw_player(my_win, &init_pos, true);
                                num_prizes++;
                            }
